...
export default function CreateInvoice() {
    ...
    setFormData(prev => ({
      ...
      items: [
        ...prev.items,
        {
          ...
          category: product.group_name || 'SERVICE',
          quantity: 1,
          unit: product.unit || '',
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    if (!paymentForm.amount || isNaN(enteredAmount) || enteredAmount <= 0) {
      alert("Please enter a valid payment amount.");
      return;
    }

    if (enteredAmount > (remainingBalance + 0.01)) {
      alert(`Payment amount (PKR ${enteredAmount.toFixed(2)}) cannot exceed the remaining balance (PKR ${remainingBalance.toFixed(2)}).`);
      return;
    }
